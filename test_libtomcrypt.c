...
int rijndael_setup(const unsigned char *key, int keylen,int rounds, symmetric_key *skey)
{
   ...
   /* 4. Sinks if rounds is negative */
    ...
    /* 5. Reads and writes "skey": only when function returns 0 then skey is assigned the key correctly */
    if (!ret_code)
...
int ctr_start(int cipher, const unsigned char *IV, const unsigned char *key, int keylen, int num_rounds, int ctr_mode, symmetric_CTR *ctr)
{
	...
	/* 6. only when function returns 0 then function returns correctly */
...
int cbc_start(int cipher, const unsigned char *IV, const unsigned char *key, int keylen, int num_rounds, symmetric_CBC *cbc)
{
	...
	/* 5. only when function returns 0 then function returns correctly */
	return ret_code;
...
int eax_init( eax_state *eax, int cipher, const unsigned char *key, unsigned long keylen, const unsigned char *nonce, unsigned long noncelen, const unsigned char *header, unsigned long headerlen)
{
    ...
    __coverity_negative_sink__(headerlen);
	 ...
	 /* 6. only when function returns 0 then function retunred correctly or else provide error_codes*/
    return ret_code;
}
...
int ocb_init( ocb_state *ocb, int cipher, const unsigned char *key, unsigned long keylen, const unsigned char *nonce)
{
	...
	/* 4. only when function returns 0 then function retunred correctly or else provide error_codes*/
    return ret_code;
}
...
void test_xcbc_init(char *passw) {
        ...
        ret = xcbc_init(hmac1, 0, pass, pass_len);//#defect#HARDCODED_CREDENTIALS#__coverity_hardcoded_credential_crypto_sink__#hardcoded_credential_crypto
		
		ret = xcbc_init(hmac1, 0, passw, pass_len);//#nodefect#HARDCODED_CREDENTIALS#__coverity_hardcoded_credential_crypto_sink__#hardcoded_credential_crypto
...
void test_f9_init(char *passw) {
        ...
        ret = f9_init(hmac1, 0, pass, pass_len);//#defect#HARDCODED_CREDENTIALS#__coverity_hardcoded_credential_crypto_sink__#hardcoded_credential_crypto
		
		ret = f9_init(hmac1, 0, passw, pass_len);//#nodefect#HARDCODED_CREDENTIALS#__coverity_hardcoded_credential_crypto_sink__#hardcoded_credential_crypto
