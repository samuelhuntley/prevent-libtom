...
void test_lrw_start(char *passw,char *tweak2) {
		...
		/* It seems "pass" is not being detected as a TP.This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */		
		ret = lrw_start(1, IV1, pass, pass_len, tweak2, 0, lrw1); //#fndefect#HARDCODED_CREDENTIALS  

		/* Again "pass" is not detected as a TP. This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */
		ret = lrw_start(1, IV1, pass, pass_len, tweak1, 0, lrw1);//#defect#HARDCODED_CREDENTIALS //#fndefect#HARDCODED_CREDENTIALS
...
void test_xts_start(char *passw,char *tweak2) {
		...
		/* It seems "key_1" is not being detected as a TP.This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */
		ret = xts_start(1, key_1, tweak2, key1_len, 0, lrw1);//#fndefect#HARDCODED_CREDENTIALS	

		/* Again "key_1" is not detected as a TP. This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */
		ret = xts_start(1, key_1, key_2, key1_len, 0, lrw1);//#defect#HARDCODED_CREDENTIALS //#fndefect#HARDCODED_CREDENTIALS
...
void test_f8_start(char *passw,char *tweak2) {
		...
		/* It seems "key_1" is not being detected as a TP.This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */
		ret = f8_start(1, IV1, key_1, key1_len, tweak2, key2_len, 0, lrw1);//#fndefect#HARDCODED_CREDENTIALS

		/* Again "key_1" is not detected as a TP. This used to work when the function definition is inlined, but fails when the definition and test case are in seperate files. Hence marking this one as FN */
		ret = f8_start(1, IV1, key_1, key1_len, key_2, key2_len, 0, lrw1);//#defect#HARDCODED_CREDENTIALS //#fndefect#HARDCODED_CREDENTIALS			
